...
const checkIsNumberCalories = (req, res, next) => {
    ...
    if (calories === undefined ||
        ...
        res.status(400).json({ error: "calories must be number type, calories validation failed" })
}
...
const checkImageLink = (req, res, next) => {
    ...
    if (/^http:\/\/commons.wikimedia.org\/wiki\/Main_Page|^https:\/\/commons.wikimedia.org\/wiki\/Main_Page/.test(image) ||
        image === null || image === "" || image === undefined)
        return next()
    else
        res.status(400).json({
            error: `image link must start as http(s)://commons.wikimedia.org/wiki/Main_Page,` +
